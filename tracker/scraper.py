    ...
    try:
        ...
        if "myworkdayjobs.com" in url:
            ...
            if job_id_match:
                ...
                host_url_match = re.search(r'https?://([a-zA-Z0-9.-]+\.(?:oraclecloud|oracle)\.com)', url)
                if host_url_match:
                    host = host_url_match.group(1)
                    
                site_number = "CX_45001"
                site_url_match = re.search(r'/sites/([^/]+)/', url)
                if site_url_match:
                    site_number = site_url_match.group(1)
                else:
                    # Fallback to HTML scrape for site number
                ...
                api_url = f"https://{host}/hcmRestApi/resources/latest/recruitingCEJobRequisitionDetails?finder=ById;Id=%22{job_id}%22,siteNumber={site_number}"
                api_res = requests.get(api_url, headers=headers, timeout=5)
                if api_res.status_code == 200:
                    data = api_res.json()
                    ...
                    if items:
                        item = items[0]
                        desc_html = " ".join([
                            item.get("ShortDescriptionStr") or "",
